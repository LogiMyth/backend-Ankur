...
app.post("/chat", async (req, res) => {
  try {
    ...
    const response = await openai.chat.completions.create({
      model: "gpt-4o-mini",
      messages: [
        { role: "system", content: "You are a friendly assistant for Social Awareness Hub." },
    ...
    return res.status(500).json({ reply: "Chatbot error" });
  }
});
...
app.post('/webhook/update', checkAdminKey, async (req, res) => {
  ...
  try {
    ...
    if (reg && reg.contact && looksLikeEmail(reg.contact) && reg.consent) {
      ...
      const trackUrl = `${process.env.SITE_ORIGIN || ('http://localhost:' + (process.env.PORT || 10000))}/?track=${encodeURIComponent(app_id)}`;
      const text = `
